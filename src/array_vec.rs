...
impl<T, const N: usize> ArrayVec<T, N> {
    ...
    /// Clears the vector; dropping elements and resetting the length to zero.
    ...
    /// Removes the last element of the vector and returns it. Returns `None` if
    /// the vector is empty.
    #[must_use]
...
impl<T, const N: usize> std::borrow::Borrow<[T]> for ArrayVec<T, N> {
    fn borrow(&self) -> &[T] {
        self.as_slice()
    }
}

impl<T, const N: usize> std::borrow::BorrowMut<[T]> for ArrayVec<T, N> {
...
where
    T: 'a + Copy,
{
    /// Extends the vector with the contents of the iterator until the iterator
    /// returns `None`, or until the vector is full.
...
mod tests {
    ...
    fn array_vec_deref() {
        ...
        // deref coercion to &mut [T]
        slice[0] = 2;

        // check that it's reflected in the ArrayVec
        assert_eq!(vec[0], 2);
    }
